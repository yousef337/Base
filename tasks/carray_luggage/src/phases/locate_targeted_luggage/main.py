...
def in_result(results, entry):
    ...
    return len(results) > 0 and entry[0] in list(map(lambda r: r[0], results)) and min(list(map(lambda r: np.linalg.norm(np.array(r[3])-np.array(entry[1])), results))) < 0.1

def analyze_area(context):
    results = []
    looks = [context.headController.look_straight, context.headController.look_left, context.headController.look_right]

    for i in looks:
        i()
        img_msg = rospy.wait_for_message('/xtion/rgb/image_raw', Image)
        pcl_msg = rospy.wait_for_message("/xtion/depth_registered/points", PointCloud2)
        detections = list(filter(lambda x: x.name in ['suitcase', 'person'], context.yolo(img_msg, "yolov8n-seg.pt", 0.7, 0.2).detected_objects))
        mappedDetections = list(map(lambda d: (d.name, estimate_xyz_from_points(context, pcl_msg, contour(context, img_msg,  np.array(d.xyseg).reshape(-1, 2)).tolist())), detections))
...
def get_hand_vectors(context, detection):
    ...
    rightElbow = estimate_xyz_from_single_point(context, detection[1], pixels[0][0], pixels[0][1])
    rightWrist = estimate_xyz_from_single_point(context, detection[1], pixels[1][0], pixels[1][1])
    leftElbow = estimate_xyz_from_single_point(context, detection[1], pixels[2][0], pixels[2][1])
    leftWrist = estimate_xyz_from_single_point(context, detection[1], pixels[3][0], pixels[3][1])
...
def calculate_angles(detections, rightVec, leftVec, rightWrist, leftWrist):
    ...
    for bag in bags:
        bagsAngles.append(get_angle_between((np.array(bag[3]) - rightWrist), rightVec))
        bagsAngles.append(get_angle_between((np.array(bag[3]) - leftWrist), leftVec))
...
def get_pointed_pose(detections, rightVec, leftVec, rightWrist, leftWrist):
    return calculate_angles(detections, rightVec, leftVec, rightWrist, leftWrist)


def main(context):
    detections = analyze_area(context)
    rightVec, leftVec, rightWrist, leftWrist = get_hand_vectors(context, detections[(list(map(lambda d: d[0], detections))).index("person")])
    context.luggagePose = get_pointed_pose(detections, rightVec, leftVec, rightWrist, leftWrist)[3]
